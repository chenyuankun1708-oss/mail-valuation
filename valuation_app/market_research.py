...
ETF_CANDIDATES = OrderedDict([
    # 宽基指数：每只白名单指数2-3只高流动性ETF
    ("000300", ["510300.XSHG", "159919.XSHE", "510350.XSHG"]),
    ("000905", ["510500.XSHG", "159922.XSHE", "512500.XSHG"]),
    ("000852", ["512100.XSHG", "159845.XSHE", "560010.XSHG"]),
    ("000510", ["563360.XSHG", "159338.XSHE", "512050.XSHG"]),
    ("000922", ["510880.XSHG", "515180.XSHG", "581000.XSHG"]),
    ("000918", ["159520.XSHE"]), ("000919", ["159521.XSHE"]),
    ("932000", ["159531.XSHE", "563300.XSHG"]),  # 中证2000
    ("399303", ["159543.XSHE"]),                  # 国证2000
    ("000985", ["510760.XSHG"]),                  # 中证全指（含细分行业ETF）
])
# 申万一级行业ETF映射：策略实验室行业配置的执行工具候选。
# 只登记真实存在、跟踪对应申万一级行业指数的行业ETF；无对应ETF的行业不登记。
INDUSTRY_ETF_CANDIDATES = OrderedDict([
    ("801010", ["512870.XSHG"]),   # 农林牧渔：畜牧养殖ETF
    ("801030", ["159865.XSHE"]),   # 基础化工：化工ETF
    ("801040", ["515220.XSHG"]),   # 钢铁：钢铁ETF
    ("801050", ["515210.XSHG"]),   # 有色金属：有色60ETF
    ("801080", ["512480.XSHG"]),   # 电子：半导体ETF
    ("801120", ["515170.XSHG"]),   # 食品饮料：食品饮料ETF
    ("801150", ["512010.XSHG"]),   # 医药生物：医药ETF
    ("801180", ["512200.XSHG"]),   # 房地产：房地产ETF
    ("801710", ["516750.XSHG"]),   # 建筑材料：建材ETF
    ("801730", ["516160.XSHG"]),   # 电力设备：新能源ETF
    ("801740", ["512810.XSHG"]),   # 国防军工：军工ETF
    ("801750", ["512720.XSHG"]),   # 计算机：计算机ETF
    ("801760", ["512980.XSHG"]),   # 传媒：传媒ETF
    ("801770", ["515880.XSHG"]),   # 通信：通信ETF
    ("801780", ["512800.XSHG"]),   # 银行：银行ETF
    ("801790", ["512000.XSHG"]),   # 非银金融：券商ETF
    ("801880", ["512400.XSHG"]),   # 汽车：新能源车ETF
    ("801960", ["159697.XSHE"]),   # 石油石化：石油天然气ETF
])
...
def update_cache(path=DEFAULT_CACHE, now=None, env_path=".env", rqdata_client=None):
    ...
    def etf_loader():
        ...
        def _load_one(code):
            """Load a single ETF candidate; return history points or None."""
            try:
                units = rqdata_client.etf.get_daily_units(code, start, end)
                nav = rqdata_client.etf.get_nav(code, start, end)
            except Exception as exc:  # single-ETF failure must not abort the module
                failures.append("%s: %s" % (code, type(exc).__name__))
                return None
            ...
            return history if len(history) >= 120 else None

        for index_code, candidates in ETF_CANDIDATES.items():
            rows = []
            for code in candidates:
                history = _load_one(code)
                if history:
        ...
        # 申万一级行业ETF：同一加载与筛选规则，供策略实验室行业配置参考
        for industry_code, candidates in INDUSTRY_ETF_CANDIDATES.items():
            rows = []
            for code in candidates:
                history = _load_one(code)
                if history:
                    rows.append({"code": code, "latest_aum": history[-1]["aum"], "history": history})
            rows.sort(key=lambda item: -item["latest_aum"])
            result["industry_" + industry_code] = {
                "selected": rows[0]["code"] if rows else None, "candidates": rows,
                "basis": "最新ETF份额×NAV规模；至少120个交易日；申万一级行业执行工具候选"}
        if failures:
            errors.append({"module": "etf_mapping", "detail": "部分ETF候选读取失败（已跳过）",
                           "failures": failures})
        return result
