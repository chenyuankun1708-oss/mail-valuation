...
本项目只做一件事：从邮箱下载指定 15 只单一资产管理计划的估值表，读取资产净值、总份额、单位净值和累计单位净值，估算申购/赎回/分红，计算单一投资人的收益与收益率，并通过本地网页展示。
...
- 收益金额：15只产品均只有一个客户，因此直接按客户持有的全部资产净值计算：`期末资产净值 - 期初资产净值 - 申购金额 + 赎回金额 + 现金分红`。
- 年化收益率：以期初资产、台账申赎、现金分红和期末资产构造现金流，计算 XIRR。
...
生成结果位于项目根目录的 `index.html`。页面可人工选择开始和结束日期；如果边界当天没有估值表，每只产品分别使用该日期之前最近的估值表。申购赎回明细读取根目录《专户资金台账.xlsx》的 `2024`、`2025`、`2026` 工作表；收益金额按期初/期末资产净值扣除台账净申购并加回现金分红计算，年化收益率按上述现金流计算 XIRR。当前总投资额读取 `专户资金余额(2026)`，会议批准额度汇总2026流水区的审核金额，不读取下方“额度/占用”汇总区。计算口径和已知限制详见 [CALCULATION_METHOD.md](CALCULATION_METHOD.md)。
...
估值表支持 `.xls` 和 `.xlsx`，可位于 `products/` 的任意子目录。系统仅接受配置中的 15 只产品，并按表头或文件名匹配。至少需要两张不同估值日的表才能计算收益。
